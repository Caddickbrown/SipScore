...
// Cider and mead used to keep sweetness in drinks.type, mixed in with kinds
// (Rosé, Sparkling…). Sweetness now lives in style; these are the old values.
const CIDER_SWEETNESS = ['Dry', 'Medium Dry', 'Medium', 'Sweet'];
const MEAD_SWEETNESS = ['Dry', 'Semi-Sweet', 'Sweet'];
const MEAD_TYPE_RENAMES = { 'Fruit Mead': 'Fruit', 'Spiced Mead': 'Spiced' };
    ...
    SELECT
      ...
      NOT EXISTS (
        ...
        WHERE (category = 'cider' AND type = ANY(${CIDER_SWEETNESS}))
           OR (category = 'mead' AND type = ANY(${[...MEAD_SWEETNESS, ...Object.keys(MEAD_TYPE_RENAMES)]}))
      ) AS sweetness_ok
  `;
  return Boolean(data && data.names_ok && data.sweetness_ok);
}
...
async function migrateSweetness(sql) {
  await sql`
    UPDATE drinks
    SET style = COALESCE(style, type), type = NULL
    WHERE (category = 'cider' AND type = ANY(${CIDER_SWEETNESS}))
       OR (category = 'mead' AND type = ANY(${MEAD_SWEETNESS}))
  `;
  for (const [from, to] of Object.entries(MEAD_TYPE_RENAMES)) {
    await sql`UPDATE drinks SET type = ${to} WHERE category = 'mead' AND type = ${from}`;
  }
}
